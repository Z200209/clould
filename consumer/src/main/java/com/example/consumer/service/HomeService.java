...
import com.example.common.utils.ResponseProcess;
...
public class HomeService {
    ...
    public Map<String, Object> getHomePageData() {
        ...
        try {
            ...
            CompletableFuture<List<TypeListVO>> channelFuture = CompletableFuture
            ...
            CompletableFuture<Map<String, Object>> productFuture = CompletableFuture
                .supplyAsync(this::getRecommendProductList, executorService);
            
            // 等待所有任务完成并获取结果
            List<Banner> banners = bannerFuture.get();
            List<TypeListVO> channels = channelFuture.get();
            List<Event> events = eventFuture.get();
            Map<String, Object> products = productFuture.get();
     ...
     * 获取频道列表 - 使用Type数据
     */
    private List<TypeListVO> getChannelList() {
        log.info("开始获取Channel数据");
        
        try {
            // 获取启用的分类作为频道
            Response<List<TypeListVO>> response = gameService.typeList(null,null);
            List<TypeListVO> types = response.getResult();
            
            // 限制返回数量，只返回前8个
            List<TypeListVO> channels = types.size() > 8 ?
     ...
     * 获取推荐商品列表 - 使用Game数据
     */
    private Map<String, Object> getRecommendProductList() {
        log.info("开始获取推荐商品数据");
        
        try {
            // 获取游戏数据作为推荐商品
            Response<Map<String, Object>> response = gameService.gameList(null,1,null,null);
            Map<String, Object> games = ResponseProcess.getData(response);
