...
public class ExcelService {
    ...
    public void exportTypeToExcel(HttpServletResponse response, User loginUser, String keyword) {
        try {
            // 获取分类数据
           Response<List<TypeListVO>> ResponseTypeList = gameService.typeList(loginUser, keyword);
           if (!ResponseProcess.isSuccess(ResponseTypeList)) {
               throw new RuntimeException("获取分类数据失败");
           }

           List<TypeListVO> typeList = ResponseProcess.getData(ResponseTypeList);
           if (typeList == null || typeList.isEmpty()) {
               throw new RuntimeException("分类数据为空");
           }
           List<TypeExcelDTO> excelData = new ArrayList<>();
           for (TypeListVO type : typeList) {
    ...
    private byte[] generateGameExcel(String  keyword, BigInteger typeId) throws IOException {
        ...
        Response<Map<String, Object>> ResponseGameList = gameService.gameList(null, 1, keyword, typeId );
        Map<String, Object> MapGameList = ResponseProcess.getData(ResponseGameList);
        if (MapGameList == null) {
            log.warn("游戏列表数据为空");
            return new byte[0];
        }
        List<Game> gameList = (List<Game>) MapGameList.get("list");
